...
# refused rather than silently forwarded without admin effect. The direct-gh
# path passes through the same released-captain-hold gate as the gh-axi path.
...
case "$PROVIDER" in
  github)
    ...
    # gh-axi drops --admin, so an authorized administrator merge is the one case
    # that must reach plain gh; every other merge keeps the gh-axi path, and the
    # outcome verification below applies identically to both. Both paths pass
    # through the released-captain-hold gate before the forge command runs.
