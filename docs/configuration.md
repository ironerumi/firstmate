...
FM_NM_CONFIG=           # test override for the no-mistakes global config path diffed by bin/fm-nm-config-keydiff.sh (default $HOME/.no-mistakes/config.yaml)
...
FM_NM_KEEPWARM_SECS=1800           # quiet interval, shared by both Claude keep-warm paths, before a real cache-warming model turn is forced: a Claude crew waiting out its own live no-mistakes run (working or parked at an attributed gate) receives one benign activation, and an idle Claude supervisor session (the main firstmate or a secondmate primary) takes one benign self-wake turn; clamped to the fixed 3000-second (50-minute) fleet cap so the turn always lands inside Claude's one-hour prompt-cache window; 0 disables both paths
FM_NM_KEEPWARM_HARNESSES=claude    # space-separated harness prefixes whose CREWS opt into that activation; every other crew harness stays an untouched no-op, and the supervisor self-wake ignores this list because it is hard Claude-only
FM_KEEPWARM_SELFWAKE_POLL_SECS=30  # seconds between a sleeping supervisor self-wake's checks that a real turn has not superseded it; test-scale knob, never a cadence
