...
Two supervision failures cannot reach the captain through a wake: an unexplained watcher cycle, and work parked on a human decision.
...
FM_AWS_SSO_CONFIG=       # alternate file for the AWS SSO refresh command instead of config/aws-sso-refresh.json ("AWS SSO refresh" above; bin/fm-aws-sso-refresh.sh --help owns the schema)
...
FM_ALERT_CHANNEL=                  # override config/supervision-alert with one directive for the unexplained-watcher-cycle and parked-work alerts; off|auto|osascript|slack|command:<cmd>; absent = auto plus slack (docs/supervision-alerts.md)
