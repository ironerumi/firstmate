...
An absent tool reports `MISSING:` with its install command, while a present-but-below-floor tool reports `OUTDATED:` with its installed version and upgrade command; [`bin/fm-bootstrap.sh`](../bin/fm-bootstrap.sh)'s header owns both exact line formats.

- An absent `tasks-axi` reports `MISSING: tasks-axi (install: npm install -g tasks-axi)` and an incompatible one reports `OUTDATED:` with that same `npm install -g tasks-axi` upgrade; when `config/backlog-backend` is not `manual`, a home with a configured non-markdown adapter or a markdown backlog refuses lifecycle mutation until compatible `tasks-axi` is on `PATH`, while a manual-backend home keeps its backlog hand-edited.
...
- An absent or below-floor `lavish-axi` reports `PRESENTATION_UNAVAILABLE` with its required floor, install command, and explicit text fallback; [`bootstrap-diagnostics`](../.agents/skills/bootstrap-diagnostics/SKILL.md) owns the response and compatibility check before visual use.
- An absent `quota-axi` reports `MISSING: quota-axi (install: npm install -g quota-axi)` and a too-old one reports `OUTDATED:` with its `quota-axi update` upgrade; firstmate cannot resolve a profile array without a compatible binary.
