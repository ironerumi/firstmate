...
| `fm-aws-sso-refresh.sh`  | Refresh an expired AWS SSO session through a safe attached Chrome tab and verify identity |
...
| `fm-guard.sh`            | Warn on primary-checkout tangles, pending queued wakes, unhealthy supervision, and unrecorded captain merge waits |
...
| `fm-nm-run-lib.sh`       | Shared branch-and-code-identity attribution for no-mistakes runs, used by fm-crew-state.sh and fm-teardown.sh |
...
| `fm-alert-lib.sh`        | Deliver the unexplained-watcher-cycle and parked-work alerts through the configured user-visible channels |
