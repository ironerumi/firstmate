...
| `fm-claude-keepwarm-selfwake.sh` | Claude Stop `asyncRewake` hook arming one deterministic self-wake per turn boundary so an idle Claude supervisor takes a benign cache-warming turn within the keep-warm cadence; cancelled by any real turn, off the watcher entirely |
...
| `fm-keepwarm-cadence-lib.sh` | Single owner of the keep-warm quiet interval and its fixed 3000-second cap, shared by the crew keep-warm and the supervisor self-wake |
...
| `fm-nm-config-keydiff.sh` | Report default config keys the installed no-mistakes ships but the operator's global config does not set |
