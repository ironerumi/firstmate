...
Claude's sibling `bin/fm-claude-keepwarm-selfwake.sh` Stop hook is independent of supervision need: it arms one deterministic self-wake per turn boundary so an idle Claude supervisor still takes a benign prompt-cache-warming turn within the shared keep-warm cadence, cancelled by any real turn and never routed through the watcher.
