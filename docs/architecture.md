...
On `attached` it stays live across identity-matched successors, and an unexplained clean child close either attaches to a verified healthy successor or is re-armed in place under a bounded repair budget, whose exhaustion is what produces the typed nonzero `watcher: FAILED - cycle ended without an actionable reason` result plus one user-visible alert; [`supervision-alerts.md`](supervision-alerts.md) owns that repair bound and its alert channels.
...
Both guards carry one further condition, a task-reported ready PR waiting on the captain with no durable captain-kind hold recording that wait; [turnend-guard.md](turnend-guard.md) owns it and `bin/fm-merge-wait-lib.sh` owns its predicate.
...
The primary checkout is healthy on its expected branch - the repository default, or a `config/primary-branch` intentionally kept there instead (docs/configuration.md) - and linked worktrees or secondmate homes are healthy at detached HEAD.
Only a named branch other than that expected branch checked out in `FM_ROOT` is a worktree tangle.

`fm-tangle-lib.sh` resolves the expected branch - a valid `config/primary-branch` when set, otherwise the default branch from `origin/HEAD`, then local `main` or `master` - and classifies any other named primary branch as the tangle.
...
Independently, `fm-spawn.sh`, `fm-task-register.sh`, `fm-send.sh`, and `fm-teardown.sh` source `bin/fm-gate-refuse-lib.sh` and exit with status 3 before fleet mutation when the gate environment marker is present or the current checkout matches the default no-mistakes gate-repository topology.
