...
The status-log CI-ready fallback additionally requires the note to contain a genuine pull-request reference, rather than treating an arbitrary `PR` substring as evidence.
...
Claude's sibling `bin/fm-claude-keepwarm-selfwake.sh` is the one keep-warm mechanism for every Claude session, supervisor and crew alike, independent of supervision need and of the watcher; [`turnend-guard.md`](turnend-guard.md#harness-integrations) owns its hook and cancellation contract.
