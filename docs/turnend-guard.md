...
`tests/fm-claude-keepwarm-selfwake.test.sh` covers the self-wake deadline, the 50-minute cap, default and disabled cadence, cancellation and re-arm, the supervisor and crew forms with independent markers, the bare form standing down in a crew worktree, and the Cursor, malformed-argument, missing-state, and missing-`jq` stand-down paths, while `tests/fm-spawn-claude-attribution.test.sh` runs the injected crew entry end to end from a spawned worktree.
