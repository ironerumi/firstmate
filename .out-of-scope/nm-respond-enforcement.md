...
The backtick hazard already has a landed mitigation: `AGENTS.md` required all `--instructions`/`--add-finding` text to be passed as `"$(cat <file>)"` from a written file (merged 2026-08-14, after the single measured incident, which predates it), and the same rule shipped in the no-mistakes skill text crews load. Each hazard has exactly one measured occurrence, and the vanished-finding case was caught by the operator.

Upstream's captain-intent contract (`--intent`, the brief's `## Captain's intent` section, `bin/fm-dod-lib.sh`) has since structurally superseded the fork's file-mediated prose rule: the accepted upstream merge dropped that `AGENTS.md` sentence and took upstream's intent wording instead. The reasoning below is kept as the historical record of why no enforcement machinery was built, not as an active fork rule.

Building the mechanism anyway was rejected because:

- **The guard cannot detect the corruption.** The validation-owner guard is a PATH shim (`bin/shims/no-mistakes` → `fm-nm-guard-shim.sh`), deliberately harness-agnostic so one mechanism covers every harness without per-harness hooks. A shim sees argv *after* shell evaluation, when corrupted inline text and clean file-mediated text are indistinguishable. The only enforceable shim rule is a blunt funnel — refuse *all* direct `--instructions` usage and require a helper (helper bypasses via an env marker before exec'ing the real binary). That is feasible without any per-harness hook wiring, but it would contradict and churn the then-landed `"$(cat <file>)"` rule, and it punishes the compliant form to catch a hazard observed once.
...
- Backtick/inline corruption of respond text recurs *despite* the then-landed `"$(cat <file>)"` rule — prose has then demonstrably failed and the shim-funnel design above is the pre-derived next step.
