...
config/keepwarm-secs  optional Claude keep-warm quiet interval in seconds, resolved after FM_NM_KEEPWARM_SECS; LOCAL, gitignored; absent = 1800, clamped to the fixed 3000-second cap, 0 disables; handed to every spawned crew and scout; inherited by secondmate homes; see docs/configuration.md "Claude keep-warm cadence"
...
state/               runtime records and signals; gitignored
  ...
  .keepwarm-selfwake .keepwarm-<id>  Claude keep-warm self-wake arm records for this home's supervisor session and each Claude crew; written only by bin/fm-claude-keepwarm-selfwake.sh, the crew record removed by teardown; never touch
