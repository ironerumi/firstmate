...
Pass any `--instructions` or `--add-finding` text as `"$(cat <file>)"` from a written file, never inline: the caller's shell command-substitutes backticked spans out of inline text and breaks inline JSON.
