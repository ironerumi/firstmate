...
config/aws-sso-refresh.json  optional private saved-account/browser selection for the common AWS SSO refresh command; LOCAL, gitignored, and not inherited; see docs/configuration.md "AWS SSO refresh"
...
config/supervision-alert config/alert-slack  optional channel directives and Slack channel/token source for the unexplained-watcher-cycle and parked-work alerts; LOCAL, gitignored; see docs/supervision-alerts.md
...
state/               volatile runtime signals; gitignored
  ...
  <id>.meta          written by fm-spawn (an ad-hoc primary-session ship is instead registered by fm-task-register.sh, section 7): window=, endpoint_task_id=, worktree=, project=, harness=, model=, effort=, kind=, mode=, yolo=, tasktmp=; an optional traceparent= only when trace context is enabled (docs/configuration.md "Trace context propagation"); kind=secondmate also records home= and projects=, plus remote_host=/remote_root=/remote_backend=/remote_herdr_session=/remote_target= for a remote route; the ad-hoc kind=adhoc records home= (harness=adhoc, no endpoint_task_id=, empty window=/worktree=/tasktmp=); a non-default runtime backend records further backend-specific fields (docs/configuration.md "Runtime backend"; bin/fm-backend.sh, section 8); fm-pr-check, including through fm-pr-merge, records one canonical pr= and the forge's pr_head= when available (GitHub pull requests and GitLab merge requests; docs/gitlab-merge-watch.md); fm-x-link appends x_request=, x_request_ts=, x_followups=, and optional x_platform=/x_reply_max_chars= for an X-mode-originated task (section 14)
  ...
  .hash-* .count-* .stale-* .stale-since-* .paused-* .wedge-escalations-* .keepwarm-* .seen-* .hb-surfaced-* .last-* .heartbeat-streak   watcher internals; never touch
...
4. For `heartbeat:`, review the whole fleet from the structured fleet view, reconcile suspicious tasks and PR state, update the backlog, watch each live Claude crewmate's remaining context for the below-50% compaction trigger, and never report an unchanged fleet as progress.
...
- Away mode cannot inspect a worker's remaining context; when model-owned supervision resumes, check every live Claude crewmate before dispatching new work.
...
Ship briefs route review ownership by delivery mode: `no-mistakes` projects receive the pipeline-led variant, while `direct-PR` and `local-only` projects default to the concise skill-led variant around one exact skill invocation that owns implementation, review, testing, documentation, and delivery mechanics.
Use `--free-form` only for a `direct-PR` or `local-only` ship task with no exact owning skill; on a `no-mistakes` project it cannot restore skill-led review ownership.
...
- `aws-sso-refresh` - load when already-authorized AWS work reaches an expired IAM Identity Center session or a worker would otherwise classify its browser approval as captain-only.
