...
#   (ax) the exact --admin invocation merges through plain gh (gh-axi drops
#       --admin) while metadata recording and the default --squash still apply
#   (ay) near-miss --admin=<value> spellings fail fast before recording
#   (az) repo overrides are still refused on the admin path
#   (ba) a task still held for the captain refuses the merge before the forge
#       is reached, and the same task merges once the hold is released, on
#       both the gh-axi path and the --admin path
set -u
...
# The merge gate refuses a task that is still held for the captain. Under the
# captain-hold lifecycle the hold is released by the captain's answer before
# this entrypoint is invoked, so a still-held task never reaches the forge, and
# releasing the hold is what unblocks the merge.
test_hold_gate_refuses_until_released() {
  ...
  grep -qxF 'pr merge 71 --repo example/repo --squash' "$case_dir/gh-axi.log" \
    || fail "hold-gate: the released task did not reach gh-axi"
  pass "fm-pr-merge refuses a still-held task and merges once the hold is released"
}

# The admin path is the captain's branch-protection override, not an exemption
# from the hold gate: a still-held task refuses before plain gh is reached.
