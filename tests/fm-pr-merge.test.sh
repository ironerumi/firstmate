...
#   (l) a GitLab MR URL resolves and merges through glab instead of erroring
#   (m) glab is addressed by the host from the URL, never an assumed one
#   (n) no merge method is imposed on GitLab, so the project's own one applies
#   (o) each pre-merge condition refuses independently, and all of them report
#   (p) a stale recorded pr_head= is reported and the live head is verified
#   (q) an unreadable merge request state refuses rather than merging blind
#   (r) glab or jq absent refuses before any state is recorded
#   (s) --sha in extra GitLab args fails fast, and still forwards on GitHub
#   (t) a GitLab refusal still leaves pr= recorded and the merge poll armed
set -u
