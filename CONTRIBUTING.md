...
A fixture that needs the real path of a tool must resolve it with `fm_real_tool` from `tests/lib.sh` rather than a bare `command -v`: a crewmate's pane puts firstmate's guard shims ahead of `git`, `rm`, `rmdir`, `unlink`, `mv`, `treehouse`, and `no-mistakes`, so a bare lookup captures the shim and a fixture that wraps that tool then trades places with it forever.
